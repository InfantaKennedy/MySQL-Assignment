...
SELECT
    ...
    DATE_FORMAT(p.purchase_date, '%d-%b-%Y') AS Purchase_Date
FROM purchases AS p
...
SELECT
    ...
    COALESCE(c.course_name, 'No Course Purchased') AS Course_Name,
    COALESCE(c.category, 'N/A') AS Course_Category,
    COALESCE(p.quantity, 0) AS Quantity_Purchased,
    FORMAT(COALESCE(c.unit_price * p.quantity, 0), 2) AS Total_Amount,
    COALESCE(DATE_FORMAT(p.purchase_date, '%d-%b-%Y'), 'N/A') AS Purchase_Date
FROM learners AS l
...
-- RIGHT JOIN
SELECT
    COALESCE(l.full_name, 'No Learner') AS Learner_Name,
    c.course_name AS Course_Name,
    c.category AS Course_Category,
    COALESCE(p.quantity, 0) AS Quantity_Purchased,
    FORMAT(COALESCE(c.unit_price * p.quantity, 0), 2) AS Total_Amount,
